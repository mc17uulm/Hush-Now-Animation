...
// soll lineIn als Quelle benutzt werden oder der Track?
boolean listenLineIn = true;
...
*/
  ...
  // line in oder track?
  if (listenLineIn) {
    grid.speed = map(in.mix.level(), 0, 1, 20, 35);
    lineInDrops();
  } else {
    // speed einstellen anhand der lautstärke des tracks
    grid.speed = map(song.mix.level(), 0, 1, 19, 29.5);
    guitarDrops();
  }
